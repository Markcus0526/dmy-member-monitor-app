...
namespace INMC.Communication.Inmc.Communication.Protocols.BinarySerialization
{
    ...
    /// </summary>
    public class BinarySerializationProtocol : IInmcWireProtocol
    {
        ...
        /// <summary>
        /// This MemoryStream object is used to collect bytes of inmc message to send
        /// </summary>
        private MemoryStream _msgBuildStream;

        /// <summary>
        /// This MemoryStream object is used to extract field datas from bytes array
        /// </summary>
        private MemoryStream _msgDestructStream;
        ...
        #region IInmcWireProtocol implementation
        ...
        public byte[] GetBytes(IInmcMessage message)
        {
        ...
        /// </returns>
        public IEnumerable<IInmcMessage> CreateMessages(byte[] receivedBytes)
        {
            ...
            var messages = new List<IInmcMessage>();
        ...
        /// <summary>
        /// This method is used to serialize a IInmcMessage to a byte array.
        ...
        /// </returns>
        protected virtual byte[] SerializeMessage(IInmcMessage message)
        {
#if (FORMATTER_SERIALIZE)
            //********************************************
            //ADDED BY JUJ. 2012.10.4.
            //********************************************
            _msgBuildStream = new MemoryStream();

            //mark field that indicate wheter this message is a text or binary message
            //Message Type
            if (message is InmcTextMessage)
                _msgBuildStream.WriteByte(0x0);
            else
                _msgBuildStream.WriteByte(0x1);

            //MessageID, RepID
            MsgDataConverter.WriteStringField(_msgBuildStream, message.MessageId);
            MsgDataConverter.WriteStringField(_msgBuildStream, message.RepliedMessageId);

            //Message Data
            if (message is InmcTextMessage)
            {
                InmcTextMessage txtMessage = (InmcTextMessage)message;
                MsgDataConverter.WriteStringField(_msgBuildStream, txtMessage.Text);
            }
            else if (message is InmcRawDataMessage)
            {
                InmcRawDataMessage rawMessage = (InmcRawDataMessage)message;
                _msgBuildStream.Write(rawMessage.MessageData, 0, rawMessage.MessageData.Length);
            }
            return _msgBuildStream.ToArray();
#else
        ...
        /// <summary>
        /// This method is used to deserialize a IInmcMessage from it's bytes.
        ...
        protected virtual IInmcMessage DeserializeMessage(byte[] bytes)
        {
#if (FORMATTER_SERIALIZE)
            //********************************************
            //ADDED BY JUJ. 2012.10.4.
            //********************************************
            _msgDestructStream = new MemoryStream();
            _msgDestructStream.Write(bytes, 0, bytes.Length);
            _msgDestructStream.Position = 0;

            bool bRawMessage;
            bRawMessage = (_msgDestructStream.ReadByte() == 1);

            //read MSG ID & REP ID
            string msgID, msgRepID;
            msgID = MsgDataConverter.ReadStringField(_msgDestructStream);
            msgRepID = MsgDataConverter.ReadStringField(_msgDestructStream);
            
            byte[] msgData = new byte [_msgDestructStream.Length - _msgDestructStream.Position];
            _msgDestructStream.Read(msgData, 0, (int)(_msgDestructStream.Length - _msgDestructStream.Position));

            if (bRawMessage)
            {
                InmcRawDataMessage rawMsg = new InmcRawDataMessage();

                rawMsg.MessageId = msgID;
                rawMsg.RepliedMessageId = msgRepID;

                //Msg Data
                rawMsg.MessageData = msgData;

                return rawMsg;
            }
            else
            {
                InmcTextMessage txtMsg = new InmcTextMessage();

                txtMsg.MessageId = msgID;
                txtMsg.RepliedMessageId = msgRepID;

                txtMsg.Text = MsgDataConverter.ReadStringField(_msgDestructStream);
                return txtMsg;
            }
#else
            ...
            using (var deserializeMemoryStream = new MemoryStream(bytes))
            {
                ...
                return (IInmcMessage)binaryFormatter.Deserialize(deserializeMemoryStream);
            }  
#endif
        ...
        /// </returns>
        /// <exception cref="CommunicationException">Throws CommunicationException if message is bigger than maximum allowed message length.</exception>
        private bool ReadSingleMessage(ICollection<IInmcMessage> messages)
        {
