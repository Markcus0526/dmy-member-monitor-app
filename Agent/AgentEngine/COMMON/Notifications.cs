...
namespace AgentEngine
{
	...
	public class ShellNotifications
	{
        ...
        #region GetSpecialFolderPath Function

        public static string GetSpecialFolderPath(CSIDL sf)
        {
            const int MAX_PATH = 260;
            StringBuilder sb = new StringBuilder(MAX_PATH);
            SHGetSpecialFolderPath(IntPtr.Zero, sb, sf, false);
            return sb.ToString();
        }
        #endregion
		...
		public ulong RegisterChangeNotify(IntPtr hWnd, CSIDL FolderID, bool Recursively)
		{
			...
			notifyid = SHChangeNotifyRegister(
                ...
                SHCNE.SHCNE_DELETE | SHCNE.SHCNE_CREATE | SHCNE.SHCNE_RENAMEITEM | SHCNE.SHCNE_MKDIR | SHCNE.SHCNE_RMDIR | SHCNE.SHCNE_RENAMEFOLDER,
        ...
        private int GetNotifyType(SHCNE shType)
        {
            int nret = -1;
            if (shType == SHCNE.SHCNE_CREATE || 
                shType == SHCNE.SHCNE_MKDIR)
                nret = 0;
            else if (shType == SHCNE.SHCNE_DELETE || 
                shType == SHCNE.SHCNE_RMDIR)
                nret = 1;
            else if (shType == SHCNE.SHCNE_RENAMEITEM || 
                shType == SHCNE.SHCNE_RENAMEFOLDER)
                nret = 2;
        ...
        public bool NotificationReceipt(IntPtr wParam, IntPtr lParam)
		{
            ...
            int nCurAction = GetNotifyType(info.Notification);
            if (lastAction != nCurAction)
            {
                lastAction = nCurAction;
                if (NotificationsReceived.Count > 0)
