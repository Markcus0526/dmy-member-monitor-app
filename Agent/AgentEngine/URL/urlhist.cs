...
namespace AgentEngine
{
	...
	public struct STATURL 
	{
		...
		public string Title
		{
			get{
				if(pwcsUrl.StartsWith("file:"))
                    return Win32.CannonializeURL(pwcsUrl, Win32.shlwapi_URL.URL_UNESCAPE).Substring(8).Replace('/', '\\');
				else
					return pwcsTitle;
			}
		}
		...
		public DateTime LastVisited
		{
			get
			{
                return Win32.FileTimeToDateTime(ftLastVisited).ToLocalTime();
			}
		}
		/// <summary>
		/// sets a column header in the DataGrid control. This property is not needed if you do not use it.
		...
		public DateTime LastUpdated
		{
			get
			{
                return Win32.FileTimeToDateTime(ftLastUpdated).ToLocalTime();
			}
		}
		/// <summary>
		/// sets a column header in the DataGrid control. This property is not needed if you do not use it.
		...
		public DateTime Expires
		{
			get
			{
				try
				{
                    return Win32.FileTimeToDateTime(ftExpires).ToLocalTime();
				}
				catch(Exception)
				{
					return DateTime.Now;
